...
#'
#' @param sz frame size of simulated square image with default 200.
...
#' @param mu mean velocity of the drift, with default 0.
...
#' show(sim_ou)
simulation <- function(sz=200, len_t=200, M=50, model_name="BM",noise="gaussian",
                       I0=20, Imax=255, pos0=matrix(NaN,nrow=M,ncol=2),
                       mu=0, rho=0.95, H=0.3, sigma_p=2, sigma_bm=1,
                       sigma_ou=2, sigma_fbm=2){
  ...
  if(!is.numeric(mu)){
    stop("Mean velocity of the drift should be numeric. \n")
  }
  if(!is.numeric(rho)){
  ...
  # Simulation particle trajectory
  if(sum(is.na(pos0))>=1){
    pos0 = matrix(sz/8+0.75*sz*stats::runif(M*2),nrow=M,ncol=2)
  }
  if(model_name == "BM"){
    pos = bm_particle_intensity(pos0=pos0,M=M,len_t=len_t,mu=mu,sigma=sigma_bm)
    model@param = c(sigma_bm)
  }else if(model_name == "OU"){
    pos = ou_particle_intensity(pos0=pos0,M=M,len_t=len_t,mu=mu,sigma=sigma_ou,rho=rho)
  ...
  if(length(I0) == len_t){
    if(noise == "uniform"){
      I = matrix(stats::runif(sz*sz*len_t)-0.5, nrow=len_t,ncol = sz*sz)
      I = I*I0
      model@sigma_2_0 = I0^2/12
    }else if(noise == "gaussian"){
      I = matrix(stats::rnorm(sz*sz*len_t), nrow=len_t,ncol = sz*sz)
      I = I*sqrt(I0)
      model@sigma_2_0 = I0
    }
  }else if(length(I0) == 1){
    if(noise == "uniform"){
      I = matrix(I0*(stats::runif(sz*sz*len_t)-0.5), nrow=len_t,ncol = sz*sz)
      model@sigma_2_0 = c(I0^2/12)
    } else if(noise == "gaussian"){
      I = matrix(sqrt(I0)*stats::rnorm(sz*sz*len_t), nrow=len_t,ncol = sz*sz)
